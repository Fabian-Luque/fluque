...
class UbicacionProp extends Model {
    ...
    public static function getLocation($prop_id) {
        return DB::select(
            'SELECT id, prop_id, X(location) as latitud, Y(location) as longitud, created_at, updated_at FROM ubicacion_propiedad WHERE prop_id=?', 
            array(
                $prop_id
            )
        );
